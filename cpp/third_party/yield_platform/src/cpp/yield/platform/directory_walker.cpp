...
DirectoryWalker::DirectoryWalker( const Path& path, const wstring& prefix  )
	: path( path )
#ifdef _WIN32
	, search_pattern( path.getWidePath() )
#endif
{
#ifdef _WIN32
	if ( search_pattern.size() > 0 && search_pattern[search_pattern.size()-1] != L'\\' ) search_pattern.append( L"\\" );
	search_pattern.append(prefix).append( L"*" );
	scan_handle = NULL;
#else
	scan_handle = opendir( path.getHostCharsetPath().c_str() );
#endif
...
bool DirectoryWalker::hasNext()
{
	...
	while ( true )
	{
		...
		wstring next_path( path.getWidePath() );
	...
	while ( ( next_dirent = readdir( ( DIR* )scan_handle ) ) )
	{
		...
		{
			if ( !prefix.empty() && string(next_dirent->d_name).find(prefix) != 0 )
				continue;


			string next_path( path.getHostCharsetPath() );
