...
#else
#include "yield/platform/platform_types.h" // For MAX_PATH
...
void Path::MultiByteToMultiByte( const char* fromcode, const string& frompath, const char* tocode, string& topath )
{
	...
	if ( ( converter = iconv_open( fromcode, tocode ) ) != ( iconv_t )-1 )
	{
		...
		if ( ::iconv( converter, NULL, NULL, ( char** )&_topath, &_topath_size ) != -1 &&
		     ::iconv( converter, ( ICONV_SOURCE_CAST )&_frompath, &_frompath_size, ( char** )&_topath, &_topath_size ) != -1 )
		{
