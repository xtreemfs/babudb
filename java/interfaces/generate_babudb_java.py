...
class BabuDBJavaBoolType(JavaBoolType, BabuDBJavaType):
...
class BabuDBJavaBufferType(JavaBufferType, BabuDBJavaType):
    def getDeclarationTypeName( self ): return "ReusableBuffer"
    def getBufferDeserializeCall( self, identifier ): return "{ %(identifier)s = org.xtreemfs.babudb.interfaces.utils.XDRUtils.deserializeSerializableBuffer( buf ); }" % locals()
    def getBufferSerializeCall( self, identifier ): return "{ org.xtreemfs.babudb.interfaces.utils.XDRUtils.serializeSerializableBuffer( %(identifier)s, writer ); }" % locals()
    def getSize( self, identifier ): return "org.xtreemfs.babudb.interfaces.utils.XDRUtils.serializableBufferLength( %(identifier)s )" % locals()


class BabuDBJavaCompoundType(BabuDBJavaType):
...
class BabuDBJavaEnumeratedType(JavaEnumeratedType, BabuDBJavaType):
...
class BabuDBJavaNumericType(JavaNumericType, BabuDBJavaType):
...
class BabuDBJavaSequenceType(JavaSequenceType, BabuDBJavaCompoundType):
    def getDeserializeMethods( self ):
        value_declaration_type = self.getValueType().getDeclarationTypeName()
        value_deserializer = self.getValueType().getBufferDeserializeCall( "new_value" )        
...
class BabuDBJavaStringType(JavaStringType, BabuDBJavaType):
    def getBufferDeserializeCall( self, identifier ): return "%(identifier)s = org.xtreemfs.babudb.interfaces.utils.XDRUtils.deserializeString( buf );" % locals()
    def getBufferSerializeCall( self, identifier ): return "org.xtreemfs.babudb.interfaces.utils.XDRUtils.serializeString( %(identifier)s, writer );" % locals()
    # def getSize( self, identifier ): return "4 + ( %(identifier)s.length() + 4 - ( %(identifier)s.length() %% 4 ) )" % locals()
    def getSize( self, identifier ): return "org.xtreemfs.babudb.interfaces.utils.XDRUtils.stringLengthPadded(%(identifier)s)" % locals() 


class BabuDBJavaStructType(JavaStructType, BabuDBJavaCompoundType):    
    def getDeserializeMethods( self ):
...
"""  % "\n".join( [INDENT_SPACES * 2 + "my_size += " + member.getType().getSize( member.getIdentifier() ) + ";" for member in self.getMembers()] )            
    ...
    def getParentTypeNames( self ):
        if len( JavaStructType.getParentTypeNames( self ) ) == 0:
            return ( None, "org.xtreemfs.babudb.interfaces.utils.Serializable" )
        else: 
...
if __name__ == "__main__":     
    ...
    generator_main( BabuDBJavaTarget )
