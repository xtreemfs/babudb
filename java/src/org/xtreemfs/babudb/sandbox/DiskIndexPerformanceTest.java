...
public class DiskIndexPerformanceTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String path = arguments.get(0) + "_" + input.substring(input.lastIndexOf("/") + 1) + "_" + "compressed_"
                + compress + "_" + size + "_" + entriesPerBlock;
        ...
        if (!(new File(path).exists()) || overwrite) {
            ...
            if (!input.equals(DEFAULT_DATAGEN)) {
                ...
                index.writeIndex(DataGenerator.randomIterator(lookupHits, size, hitrate, minStrLen, maxStrLen, minChar,
                        maxChar));
            }
        } else {
        ...
        while (warmups-- > 0) {
            ...
            while (it.hasNext() && readEntries-- > 0)
                it.next();
            diskIndex.destroy();
        }
